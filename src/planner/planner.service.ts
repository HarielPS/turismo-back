...
// async generatePlanAutomatic(id_usuario: string, fecha_inicio: string, fecha_fin: string, presupuesto?: Presupuesto) {
...
//     if (presupuesto) {
//       payload.presupuesto = [presupuesto]; // En el formato que espera Flask
//     }
...
export class PlannerService {
  ...
  async generatePlanAutomatic(
    ...
    try {
      ...
      console.log('[NestJS][5/5] Enviando a Flask...');
      const response = await firstValueFrom(
        this.http.post('http://127.0.0.1:5000/api/plan-itinerario', payload)
      );
      ...
      console.error('[NestJS] Error en generatePlanAutomatic:', error.message);
      console.error(error.stack);
      throw new InternalServerErrorException(`Error al generar plan: ${error.message}`);
    }
  }
}
